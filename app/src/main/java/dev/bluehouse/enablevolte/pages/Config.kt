...
fun Config(
    ...
    var simSlotIndex by rememberSaveable { mutableIntStateOf(-1) }
    val scope = rememberCoroutineScope()

    // ── Save current UI state to SharedPreferences ────────────────────────────
    // Must be declared before the launcher lambdas that reference it.
    fun saveCurrentSettings() {
        if (simSlotIndex < 0) return
        repo.saveSlotSettings(
            ...
            SubscriptionSettings(
                ...
                showIMSinSIMInfo = showIMSinSIMInfo,
                allowAddingAPNs = allowAddingAPNs,
                showVoWifiMode = showVoWifiMode,
                showVoWifiRoamingMode = showVoWifiRoamingMode,
                wfcSpnFormatIndex = wfcSpnFormatIndex,
                showVoWifiIcon = showVoWifiIcon,
                alwaysDataRATIcon = alwaysDataRATIcon,
                supportWfcWifiOnly = supportWfcWifiOnly,
                vtEnabled = vtEnabled,
                ssOverUtEnabled = ssOverUtEnabled,
                ssOverCDMAEnabled = ssOverCDMAEnabled,
                show4GForLteEnabled = show4GForLteEnabled,
                hideEnhancedDataIconEnabled = hideEnhancedDataIconEnabled,
                is4GPlusEnabled = is4GPlusEnabled,
    ...
    fun loadFlags() {
        Log.d(TAG, "loadFlags")
        simSlotIndex = try { moder.simSlotIndex } catch (e: Exception) { -1 }
        configurableItems =
        ...
        // Persist whatever the system currently reports
        saveCurrentSettings()
    }
